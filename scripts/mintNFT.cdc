...
import BobizNFT from "../contracts/BobizNFT.cdc"
...
transaction(
    ...
    prepare(signer: AuthAccount) {
        self.mintingIDBefore = BobizNFT.totalSupply

        // borrow a reference to the NFTMinter resource in storage
        self.minter = signer.borrow<&BobizNFT.NFTMinter>(from: BobizNFT.MinterStoragePath)
            ?? panic("Account does not store an object at the specified path")

        // Borrow the recipient's public NFT collection reference
        self.recipientCollectionRef = getAccount(recipient)
            .getCapability(BobizNFT.CollectionPublicPath)
    ...
    post {
        ...
        BobizNFT.totalSupply == self.mintingIDBefore + 1: "The total supply should have been increased by 1"
    }
}
